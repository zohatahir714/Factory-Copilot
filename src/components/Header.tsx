        ...
        <div ref={searchRef} className="relative flex-1 max-w-xl mx-2 lg:mx-6">
          ...
          {isSearchOpen && searchQuery.trim() && (
            <div className="absolute top-full mt-2 left-0 right-0 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 max-h-96 overflow-y-auto z-50 animate-fadeIn text-xs divide-y divide-slate-100 dark:divide-slate-700">
              <div className="p-2.5 bg-slate-50 dark:bg-slate-800 flex items-center justify-between text-slate-500 dark:text-slate-400">
              ...
              {totalResults === 0 ? (
                ...
                <div className="py-1">
                  ...
                  {filteredSales.length > 0 && (
                    <div className="p-2">
                      <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-2 py-1 flex items-center gap-1">
                        <Receipt className="w-3 h-3 text-indigo-600" /> Sales Orders & GST Invoices ({filteredSales.length})
                      </div>
                      {filteredSales.map(so => (
                        ...
                        >
                          <div>
                            <span className="font-bold font-mono text-indigo-600 dark:text-indigo-400">{so.invoiceNumber}</span>
                  ...
                  {filteredCompliance.length > 0 && (
                    <div className="p-2">
                      <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-2 py-1 flex items-center gap-1">
                        <Scale className="w-3 h-3 text-indigo-600" /> FBR Statutory Legal Rules ({filteredCompliance.length})
                      </div>
                      {filteredCompliance.map(cs => (
                        ...
                        >
                          <div>
                            ...
                            <span className="text-indigo-600 dark:text-indigo-400 ml-2 font-mono text-[11px]">{cs.section}</span>
                          </div>
                          <span className="text-[10px] bg-indigo-100 dark:bg-indigo-950/50 text-indigo-800 dark:text-indigo-300 px-1.5 py-0.5 rounded font-mono">
                            {cs.authority}
                          </span>
                        </div>
                      ))}
            ...
            {isQuickMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#1a1b23] rounded-2xl shadow-xl border border-transparent dark:border-white/10 py-1.5 z-50 animate-fadeIn text-xs">
                ...
                <button
                  ...
                  className="w-full px-4 py-2 text-left hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2 text-indigo-700 dark:text-indigo-300 font-semibold cursor-pointer"
                >
